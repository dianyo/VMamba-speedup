...
do
    ...
    python3 main_no_dist.py --cfg configs/vssm/vmambav2v_tiny_224.yaml --data-path /joe/data/ImageNet-Zip --output /tmp/ --resume ckpt/vssm1_tiny_0230s_ckpt_epoch_264.pth --zip > $file_name 2>&1

    # echo "Running small"
    file_name="quatermap_results/vmamba_outputs/${QUATERMAP_STRATEGY}_vmambav2v_small_224_freq_$i.log"
    python3 main_no_dist.py --cfg configs/vssm/vmambav2_small_224.yaml --data-path /joe/data/ImageNet-Zip --output /tmp/ --resume ckpt/vssm_small_0229_ckpt_epoch_222.pth --zip > $file_name 2>&1
