...
def quatermap_swin_forward_hook(module, input, output):
    ...
    feature_map = output.reshape(-1, dim, dim, output.shape[2]).permute(0, 3, 1, 2)
...
def patch_swin_attention_forward(self, x, mask=None):
    B, N, C = x.shape
    qkv = (
        self.qkv(x)
    ...
    if self.window_size[0] != attn.shape[-2] or self.window_size[1] != attn.shape[-1]:
        ...
        if isinstance(module, WindowMSA):
            ...
            if layer > 2 and layer % 3 == 0:
                ...
                module.register_forward_pre_hook(
                    quatermap_swin_forward_pre_hook, with_kwargs=True
                )
